...
// function getFilterValues(forEvaluation = false) {
...
//   const state = stateSelect && stateSelect.value && !stateSelect.value.includes("All India") ? stateSelect.value : "";
...
//   const gender = activeGenderBtn && activeGenderBtn.dataset.gender !== "all" ? activeGenderBtn.dataset.gender : undefined;
...
//   const { q, state, category, gender } = getFilterValues(false);
...
function renderCard(scheme, matchInfo = null) {
  const docs = (scheme.documentsList || scheme.documents || [])
    .slice(0, 3)
    .map(
      (doc, i) => `
      <li class="doc-item" data-scheme="${scheme.id}" data-doc="${i}">
        <span class="doc-check" aria-hidden="true"></span>
        <span>${escapeHTML(typeof doc === 'string' ? doc : doc.label)}</span>
      </li>`
    )
    .join("");
  ...
  const matchBadge = matchInfo
    ? `<span class="eligible" style="background: #e6f7ef; color: #1f7a55; font-size: 11px; font-weight: 700; padding: 3px 8px; border-radius: 6px;">✓ ${matchInfo.matchPercentage}% Match</span>`
    : `<span class="status-badge ${scheme.isExpired ? 'status-expired' : 'status-active'}">${scheme.isExpired ? 'Expired' : 'Active & Open'}</span>`;
  return `
    <article class="scheme-card" data-id="${scheme.id || scheme.slug}">
      <div class="card-top">
        <div class="card-meta">
          <span class="cat-tag">${escapeHTML(scheme.primaryCategory || (scheme.categories && scheme.categories[0]) || scheme.category || 'Welfare')}</span>
          <span class="loc-tag">${escapeHTML(scheme.state || scheme.level || 'All India')}</span>
        </div>
        ${matchBadge}
      </div>
      <h3>${escapeHTML(scheme.title || scheme.scheme_name)}</h3>
      <p class="scheme-desc">${escapeHTML(scheme.details || scheme.description || '')}</p>
      ${benefitsSnippet}
      <div>
      ...
      <div class="card-actions">
        <a class="btn btn-apply" href="assistant.html?q=${encodeURIComponent('How do I apply for ' + (scheme.title || scheme.scheme_name) + ' with exact steps and documents?')}" target="_self">How to Apply ${icons.external}</a>
        <a class="btn btn-ai" href="assistant.html?q=${encodeURIComponent('Explain ' + (scheme.title || scheme.scheme_name) + ' eligibility and calculate subsidy')}">${icons.chat} Ask AI Agent</a>
      </div>
      <div class="card-footer">
        <button type="button" class="footer-link save-btn${isSaved(scheme.id || scheme.slug) ? " saved" : ""}" data-scheme="${scheme.id || scheme.slug}">${icons.bookmark} ${isSaved(scheme.id || scheme.slug) ? "Saved" : "Save"}</button>
        <a class="footer-link" href="assistant.html?q=${encodeURIComponent('What is the deadline for ' + (scheme.title || scheme.scheme_name) + '?')}">${icons.clock} Deadlines</a>
      </div>
    </article>`;
}
function renderSchemes(list, matchesMap = null) {
  const grid = document.getElementById("schemeGrid");
  if (!list || list.length === 0) {
    grid.innerHTML = `<div class="empty-state" style="grid-column: 1 / -1; padding: 40px; text-align: center; background: #fff; border-radius: 12px;">
      <h3>No matching schemes found</h3>
      <p style="color: #64748b; margin-top: 8px;">Try adjusting your search terms, state, or filters.</p>
    </div>`;
    return;
  }
  grid.innerHTML = list.map(item => renderCard(item, matchesMap ? matchesMap.get(item.id || item.slug) : null)).join("");
}
function updateAgeSlider() {
  const age = document.getElementById("age");
  const ageValue = document.getElementById("ageValue");
...
async function fetchSchemes(query = '', state = 'All', category = 'All', page = 1) {
  try {
    ...
    totalSchemesCount = data.total || 3400;

    document.getElementById("resultCount").textContent = String(totalSchemesCount);
...
async function evaluateMatches() {
  ...
  btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L3 14h8l-1 8 10-12h-8l1-8z"/></svg> Matching against 3,400+ schemes…`;
  btn.disabled = true;
  ...
  try {
    const res = await fetch('/api/schemes/match', {
      ...
      body: JSON.stringify({
        state: state.includes('All India') ? 'All India' : state,
        age: parseInt(age, 10),
        gender: gender,
        category: category.includes('All Categories') ? '' : category,
        occupation: category
      })
    });
    const data = await res.json();
    ...
    if (data.matches && data.matches.length > 0) {
      ...
      document.getElementById("resultCount").textContent = String(data.totalMatches);
      renderSchemes(schemesList, matchesMap);
    } else {
      fetchSchemes();
    }
  } catch (err) {
...
function init() {
  ...
  fetchSchemes();
  updateAgeSlider();
  document.getElementById("age").addEventListener("input", updateAgeSlider);
  document.querySelectorAll(".seg-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".seg-btn").forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
  ...
  document.getElementById("resetFilters").addEventListener("click", () => {
    ...
    updateAgeSlider();
    document.querySelectorAll(".seg-btn").forEach((b) => b.classList.remove("active"));
    document.querySelector('.seg-btn[data-gender="all"]').classList.add("active");
    document.getElementById("search").value = "";
    fetchSchemes();
  });
  document.getElementById("schemeGrid").addEventListener("click", (e) => {
    const docItem = e.target.closest(".doc-item");
    if (docItem) {
      docItem.classList.toggle("checked");
      return;
    ...
    if (saveBtn) {
      ...
      upsertSavedApp({ id, status: "saved" });
  ...
  const runSearch = () => {
    ...
    fetchSchemes(q, state, cat);
  };
  document.querySelector(".btn-search").addEventListener("click", runSearch);
  searchInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") runSearch();
  });
  document.querySelector(".btn-evaluate").addEventListener("click", evaluateMatches);
